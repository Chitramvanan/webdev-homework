...
const mapStarships = (input) => {
  ...
  return input.map(
    (item) =>
      ...
      item.manufacturer +
      " - cost: " +
      Number(item.cost_in_credits).toLocaleString("en-US") +
      " credits"
  );
};
...
const filterStarships = (input) => {
  ...
  return input.filter((item) => item.passengers < 10 && item.crew > 1);
};

const reduceStarships = (input) => {
  // Return the cost to purchase all ships in the input array
  starships_new = input.filter((item) => item.cost_in_credits != "unknown");
  const cost = starships_new.reduce(
    (acc, curr) => acc + Number(curr.cost_in_credits),
    0
  );
  return (
    "The cost of all starships is " + cost.toLocaleString("en-US") + " credits"
  );
};
